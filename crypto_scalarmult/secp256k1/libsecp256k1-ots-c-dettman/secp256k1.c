...
#error                                                                         \
    "secp256k1.h processed without SECP256K1_BUILD defined while building secp256k1.c"
#endif
...
size_t secp256k1_context_preallocated_size(unsigned int flags) {
  ...
  /* A return value of 0 is reserved as an indicator for errors when we call
   * this function internally. */
  VERIFY_CHECK(ret != 0);

  if (EXPECT((flags & SECP256K1_FLAGS_TYPE_MASK) !=
...
secp256k1_context *secp256k1_context_preallocated_create(void *prealloc,
  ...
  VERIFY_CHECK((flags & SECP256K1_FLAGS_TYPE_MASK) ==
               SECP256K1_FLAGS_TYPE_CONTEXT);
...
secp256k1_context *secp256k1_context_create(unsigned int flags) {
  ...
  secp256k1_context *ctx = (secp256k1_context *)checked_malloc(
      &default_error_callback, prealloc_size);
...
secp256k1_context *
secp256k1_context_preallocated_clone(const secp256k1_context *ctx,
                                     void *prealloc) {
  secp256k1_context *ret;
  VERIFY_CHECK(ctx != NULL);
...
secp256k1_context *secp256k1_context_clone(const secp256k1_context *ctx) {
  ...
  ret =
      (secp256k1_context *)checked_malloc(&ctx->error_callback, prealloc_size);
...
void secp256k1_scratch_space_destroy(const secp256k1_context *ctx,
                                     secp256k1_scratch_space *scratch) {
  VERIFY_CHECK(ctx != NULL);
  secp256k1_scratch_destroy(&ctx->error_callback, scratch);
}

/* Mark memory as no-longer-secret for the purpose of analysing constant-time
 * behaviour of the software. This is setup for use with valgrind but could be
 * substituted with the appropriate instrumentation for other analysis tools.
 */
static SECP256K1_INLINE void secp256k1_declassify(const secp256k1_context *ctx,
                                                  const void *p, size_t len) {
#if defined(VALGRIND)
  if (EXPECT(ctx->declassify, 0))
...
int secp256k1_ec_pubkey_serialize(const secp256k1_context *ctx,
  ...
  ARG_CHECK(*outputlen >=
            ((flags & SECP256K1_FLAGS_BIT_COMPRESSION) ? 33u : 65u));
  len = *outputlen;
  *outputlen = 0;
  ARG_CHECK(output != NULL);
  memset(output, 0, len);
  ARG_CHECK(pubkey != NULL);
  ARG_CHECK((flags & SECP256K1_FLAGS_TYPE_MASK) ==
            SECP256K1_FLAGS_TYPE_COMPRESSION);
  if (secp256k1_pubkey_load(ctx, &Q, pubkey)) {
    ret = secp256k1_eckey_pubkey_serialize(
        &Q, output, &len, flags & SECP256K1_FLAGS_BIT_COMPRESSION);
    if (ret) {
      *outputlen = len;
    }
  }
...
int secp256k1_ec_pubkey_cmp(const secp256k1_context *ctx,
                            const secp256k1_pubkey *pubkey0,
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (!secp256k1_ec_pubkey_serialize(ctx, out[i], &out_size, pk[i],
...
static void
secp256k1_ecdsa_signature_load(const secp256k1_context *ctx,
...
static int nonce_function_rfc6979(unsigned char *nonce32,
   ...
   * - the private key (32 bytes) and reduced message (32 bytes), see RFC
   * 6979 3.2d.
...
static int secp256k1_ecdsa_sign_inner(const secp256k1_context *ctx,
                                      secp256k1_scalar *r, secp256k1_scalar *s,
                                      int *recid, const unsigned char *msg32,
  ...
  /* Default initialization here is important so we won't pass uninit values to
   * the cmov in the end */
  ...
  *s = secp256k1_scalar_zero;
  ...
  while (1) {
    ...
    /* The nonce is still secret here, but it being invalid is is less likely
     * than 1:2^255. */
    secp256k1_declassify(ctx, &is_nonce_valid, sizeof(is_nonce_valid));
    if (is_nonce_valid) {
      ret = secp256k1_ecdsa_sig_sign(&ctx->ecmult_gen_ctx, r, s, &sec, &msg,
                                     &non, recid);
      /* The final signature is no longer a secret, nor is the fact that we were
       * successful or not. */
...
int secp256k1_ecdsa_sign(const secp256k1_context *ctx,
  ...
  ret = secp256k1_ecdsa_sign_inner(ctx, &r, &s, NULL, msghash32, seckey,
                                   noncefp, noncedata);
...
static int secp256k1_ec_pubkey_create_helper(
    const secp256k1_ecmult_gen_context *ecmult_gen_ctx,
    secp256k1_scalar *seckey_scalar, secp256k1_ge *p,
    const unsigned char *seckey) {
  secp256k1_gej pj;
  int ret;
...
int secp256k1_ec_pubkey_create(const secp256k1_context *ctx,
  ...
  ret = secp256k1_ec_pubkey_create_helper(&ctx->ecmult_gen_ctx, &seckey_scalar,
                                          &p, seckey);
...
static int secp256k1_ec_seckey_tweak_add_helper(secp256k1_scalar *sec,
...
static int secp256k1_ec_pubkey_tweak_add_helper(secp256k1_ge *p,
                                                const unsigned char *tweak32) {
  secp256k1_scalar term;
  int overflow = 0;
  secp256k1_scalar_set_b32(&term, tweak32, &overflow);
...
int secp256k1_tagged_sha256(const secp256k1_context *ctx, unsigned char *hash32,
