...
/* If SECP256K1_{LITTLE,BIG}_ENDIAN is not explicitly provided, infer from
 * various other system macros. */
 ...
 */
    ...
    defined(_M_AMD64) || defined(_M_ARM) /* MSVC */
 ...
 */
static SECP256K1_INLINE int secp256k1_memcmp_var(const void *s1, const void *s2,
                                                 size_t n) {
 ...
 * Both *r and *a must be initialized and non-negative.*/
static SECP256K1_INLINE void secp256k1_int_cmov(int *r, const int *a,
                                                int flag) {
  ...
  /* Casting a negative int to unsigned and back to int is implementation
   * defined behavior */
...
/* If USE_FORCE_WIDEMUL_{INT128,INT64} is set, use that wide multiplication
 * implementation. Otherwise use the presence of __SIZEOF_INT128__ to decide.
 */
...
static SECP256K1_INLINE int secp256k1_ctz32_var_debruijn(uint32_t x) {
  static const uint8_t debruijn[32] = {
      0x00, 0x01, 0x02, 0x18, 0x03, 0x13, 0x06, 0x19, 0x16, 0x04, 0x14,
      0x0A, 0x10, 0x07, 0x0C, 0x1A, 0x1F, 0x17, 0x12, 0x05, 0x15, 0x09,
      0x0F, 0x0B, 0x1E, 0x11, 0x08, 0x0E, 0x1D, 0x0D, 0x1C, 0x1B};
...
static SECP256K1_INLINE int secp256k1_ctz32_var(uint32_t x) {
  ...
  /* If the unsigned type is sufficient to represent the largest uint32_t,
   * consider __builtin_ctz. */
  if (((unsigned)UINT32_MAX) == UINT32_MAX) {
    return __builtin_ctz(x);
  }
#endif
#if (__has_builtin(__builtin_ctzl) || SECP256K1_GNUC_PREREQ(3, 4))
  /* Otherwise consider __builtin_ctzl (the unsigned long type is always at
   * least 32 bits). */
  return __builtin_ctzl(x);
#else
  /* If no suitable CTZ builtin is available, use a (variable time) software
   * emulation. */
...
static SECP256K1_INLINE int secp256k1_ctz64_var(uint64_t x) {
  VERIFY_CHECK(x != 0);
#if (__has_builtin(__builtin_ctzl) || SECP256K1_GNUC_PREREQ(3, 4))
  /* If the unsigned long type is sufficient to represent the largest uint64_t,
   * consider __builtin_ctzl. */
  if (((unsigned long)UINT64_MAX) == UINT64_MAX) {
    return __builtin_ctzl(x);
  }
#endif
#if (__has_builtin(__builtin_ctzll) || SECP256K1_GNUC_PREREQ(3, 4))
  /* Otherwise consider __builtin_ctzll (the unsigned long long type is always
   * at least 64 bits). */
