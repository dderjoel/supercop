...
#define SECP256K1_G_ORDER_199                                                  \
  SECP256K1_GE_CONST(0x226e653f, 0xc8df7744, 0x9bacbf12, 0x7d1dcbf9,           \
...
static const secp256k1_fe secp256k1_fe_const_b =
    SECP256K1_FE_CONST(0x3d3486b2, 0x159a9ca5, 0xc75638be, 0xb23a69bc,
                       0x946a45ab, 0x24801247, 0xb4ed2b8e, 0x26b6a417);
#elif EXHAUSTIVE_TEST_ORDER == 199
static const secp256k1_ge secp256k1_ge_const_g = SECP256K1_G_ORDER_199;

static const secp256k1_fe secp256k1_fe_const_b =
    SECP256K1_FE_CONST(0x2cca28fa, 0xfc614b80, 0x2a3db42b, 0x00ba00b1,
...
static void secp256k1_ge_table_set_globalz(size_t len, secp256k1_ge *a,
...
static SECP256K1_INLINE void secp256k1_gej_double(secp256k1_gej *r,
   ...
   * slower, mainly because it requires more normalizations.
   */
...
static void secp256k1_gej_double_var(secp256k1_gej *r, const secp256k1_gej *a,
   ...
   * because if 2Q = infinity, Q must equal -Q, or that Q.y == -(Q.y), or Q.y is
   * 0. For a point on y^2 = x^3 + 7 to have y=0, x^3 must be -7 mod p. However,
   * -7 has no cube root mod p.
   *
   *  Having said this, if this function receives a point on a sextic twist,
   * e.g. by a fault attack, it is possible for y to be 0. This happens for y^2
   * = x^3 + 6, since -6 does have a cube root mod p. For this point, this
   * function will not set the infinity flag even though the point doubles to
   * infinity, and the result point will be gibberish (z = 0 but infinity = 0).
   */
  if (a->infinity) {
...
static void secp256k1_gej_add_ge(secp256k1_gej *r, const secp256k1_gej *a,
   ...
   * Attacks. In D. Naccache and P. Paillier, Eds., Public Key Cryptography,
   * vol. 2274 of Lecture Notes in Computer Science, pages 335-345.
   * Springer-Verlag, 2002. we find as solution for a unified addition/doubling
   * formula: lambda = ((x1 + x2)^2 - x1 * x2 + a) / (y1 + y2), with a = 0 for
   * secp256k1's curve equation. x3 = lambda^2 - (x1 + x2) 2*y3 = lambda * (x1 +
   * x2 - 2 * x3) - (y1 + y2).
   *
...
secp256k1_gej_cmov(secp256k1_gej *r, const secp256k1_gej *a, int flag) {
...
#else
  (void)ge;
  /* The real secp256k1 group has cofactor 1, so the subgroup is the entire
   * curve. */
  return 1;
#endif
}
