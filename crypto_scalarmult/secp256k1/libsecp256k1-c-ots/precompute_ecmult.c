/*****************************************************************************************************
 * Copyright (c) 2013, 2014, 2017, 2021 Pieter Wuille, Andrew Poelstra, Jonas
 *Nick, Russell O'Connor * Distributed under the MIT software license, see the
...
/* Autotools creates libsecp256k1-config.h, of which ECMULT_WINDOW_SIZE is
   needed.
   ifndef guard so downstream users can define their own if they do not use
   autotools. */
...
static void print_table(FILE *fp, const char *name, int window_g,
  ...
  fprintf(fp,
          ...
          name);
  fprintf(fp,
          " S(%" PRIx32 ",%" PRIx32 ",%" PRIx32 ",%" PRIx32 ",%" PRIx32
...
static void print_two_tables(FILE *fp, int window_g) {
  secp256k1_ge_storage *table =
      malloc(ECMULT_TABLE_SIZE(window_g) * sizeof(secp256k1_ge_storage));
  secp256k1_ge_storage *table_128 =
      malloc(ECMULT_TABLE_SIZE(window_g) * sizeof(secp256k1_ge_storage));

  secp256k1_ecmult_compute_two_tables(table, table_128, window_g,
...
int main(void) {
  ...
  fprintf(fp, "/* This file contains an array secp256k1_pre_g with odd "
              "multiples of the base point G and\n");
  fprintf(fp, " * an array secp256k1_pre_g_128 with odd multiples of 2^128*G "
  ...
  fprintf(fp, "#define S(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p) "
              "SECP256K1_GE_STORAGE_CONST(0x##a##u,0x##b##u,0x##c##u,0x##d##u,"
              "0x##e##u,0x##f##u,0x##g##u,0x##h##u,0x##i##u,0x##j##u,0x##k##u,"
              "0x##l##u,0x##m##u,0x##n##u,0x##o##u,0x##p##u)\n");
  fprintf(fp, "#if ECMULT_WINDOW_SIZE > %d\n", window_g);
  fprintf(fp, "   #error configuration mismatch, invalid ECMULT_WINDOW_SIZE. "
              "Try deleting precomputed_ecmult.c before the build.\n");
  fprintf(fp, "#endif\n");
  fprintf(fp, "#ifdef EXHAUSTIVE_TEST_ORDER\n");
  fprintf(fp, "#    error Cannot compile precomputed_ecmult.c in exhaustive "
              "test mode\n");
