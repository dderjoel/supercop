...
static void secp256k1_scalar_split_lambda(secp256k1_scalar *r1,
  ...
  *r1 = (*k + (EXHAUSTIVE_TEST_ORDER - *r2) * EXHAUSTIVE_TEST_LAMBDA) %
...
static const secp256k1_scalar secp256k1_const_lambda = SECP256K1_SCALAR_CONST(
    0x5363AD4CUL, 0xC05C30E0UL, 0xA5261C02UL, 0x8812645AUL, 0x122E22EAUL,
    0x20816678UL, 0xDF02967CUL, 0x1B23BD72UL);

#ifdef VERIFY
static void secp256k1_scalar_split_lambda_verify(const secp256k1_scalar *r1,
                                                 const secp256k1_scalar *r2,
                                                 const secp256k1_scalar *k);
#endif

/*
 * Both lambda and beta are primitive cube roots of unity.  That is lamba^3 == 1
 * mod n and beta^3 == 1 mod p, where n is the curve order and p is the field
 * order.
 *
 * Furthermore, because (X^3 - 1) = (X - 1)(X^2 + X + 1), the primitive cube
 * roots of unity are roots of X^2 + X + 1.  Therefore lambda^2 + lamba == -1
 * mod n and beta^2 + beta == -1 mod p. (The other primitive cube roots of unity
 * are lambda^2 and beta^2 respectively.)
 *
 * Let l = -1/2 + i*sqrt(3)/2, the complex root of X^2 + X + 1. We can define a
 * ring homomorphism phi : Z[l] -> Z_n where phi(a + b*l) == a + b*lambda mod n.
 * The kernel of phi is a lattice over Z[l] (considering Z[l] as a Z-module).
 * This lattice is generated by a reduced basis {a1 + b1*l, a2 + b2*l} where
 *
 * - a1 =
 ...
 * an algorithm (algorithm 3.74) to find k1 and k2 given k, such that k1 + k2 *
 * lambda == k mod n, and k1 and k2 are small in absolute value.
 *
 * The algorithm computes c1 = round(b2 * k / n) and c2 = round((-b1) * k / n),
 * and gives k1 = k - (c1*a1 + c2*a2) and k2 = -(c1*b1 + c2*b2). Instead, we use
 * modular arithmetic, and compute r2 = k2 mod n, and r1 = k1 mod n = (k - r2 *
 * lambda) mod n, avoiding the need for the constants a1 and a2.
 *
 * g1, g2 are precomputed constants used to replace division with a rounded
 * multiplication when decomposing the scalar for an endomorphism-based point
 * multiplication.
 *
 * The possibility of using precomputed estimates is mentioned in "Guide to
 * Elliptic Curve Cryptography" (Hankerson, Menezes, Vanstone) in section 3.5.
 *
 * The derivation is described in the paper "Efficient Software Implementation
 * of Public-Key Cryptography on Sensor Networks Using the MSP430X
 * Microcontroller" (Gouvea, Oliveira, Lopez), Section 4.3 (here we use a
 * somewhat higher-precision estimate): d = a1*b2 - b1*a2 g1 = round(2^384 *
 * b2/d) g2 = round(2^384 * (-b1)/d)
 *
 * (Note that d is also equal to the curve order, n, here because [a1,b1] and
 * [a2,b2] can be found as outputs of the Extended Euclidean Algorithm on inputs
 * n and lambda).
 *
...
static void secp256k1_scalar_split_lambda(secp256k1_scalar *r1,
  ...
  static const secp256k1_scalar g1 = SECP256K1_SCALAR_CONST(
      0x3086D221UL, 0xA7D46BCDUL, 0xE86C90E4UL, 0x9284EB15UL, 0x3DAA8A14UL,
      0x71E8CA7FUL, 0xE893209AUL, 0x45DBB031UL);
  static const secp256k1_scalar g2 = SECP256K1_SCALAR_CONST(
      0xE4437ED6UL, 0x010E8828UL, 0x6F547FA9UL, 0x0ABFE4C4UL, 0x221208ACUL,
      0x9DF506C6UL, 0x1571B4AEUL, 0x8AC47F71UL);
  VERIFY_CHECK(r1 != k);
  VERIFY_CHECK(r2 != k);
