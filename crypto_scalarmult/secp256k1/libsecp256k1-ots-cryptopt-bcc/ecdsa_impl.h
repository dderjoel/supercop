 ...
 * Cryptography" (SEC2) 2.7.1 sage: for t in xrange(1023, -1, -1):
 *     ..   p = 2**256 - 2**32 - t
 *     ..   if p.is_prime():
 *     ..     print '%x'%p
 *     ..     break
...
static int secp256k1_der_read_len(size_t *len, const unsigned char **sigp,
...
static int secp256k1_ecdsa_sig_parse(secp256k1_scalar *rr, secp256k1_scalar *rs,
  ...
  if (sig == sigend || *(sig++) != 0x30) {
    /* The encoding doesn't start with a constructed sequence
     * (X.690-0207 8.9.1). */
    return 0;
  }
  if (secp256k1_der_read_len(&rlen, &sig, sigend) == 0) {
...
static int secp256k1_ecdsa_sig_verify(const secp256k1_scalar *sigr,
   ...
   * (modulo n) in xr. Naively, we would extract the x coordinate from pr
   * (requiring a inversion modulo p), compute the remainder modulo n, and
   * compare it to xr. However:
   *
   *        xr == X(pr) mod n
   ...
   * pr.z^2 mod p) [Multiplying both sides of the equations by pr.z^2 mod p]
   *    <=> (xr * pr.z^2 mod p == pr.x) || (xr + n < p && (xr + n) * pr.z^2 mod
   * p == pr.x)
   *
   *  Thus, we can avoid the inversion, but we have to check both cases
   * separately. secp256k1_gej_eq_x implements the (xr * pr.z^2 mod p == pr.x)
   * test.
   */
...
static int secp256k1_ecdsa_sig_sign(const secp256k1_ecmult_gen_context *ctx,
  ...
  if (recid) {
     ...
     * requires finding the discrete log of some P where P.x >= order, and only
     * 1 in about 2^127 points meet this criteria.
     */
  ...
  /* P.x = order is on the curve, so technically sig->r could end up being zero,
   * which would be an invalid signature. This is cryptographically unreachable
   * as hitting it requires finding the discrete log of P.x = N.
   */
  return (int)(!secp256k1_scalar_is_zero(sigr)) &
